#!/usr/bin/env node
/**
 * 测试脚本：验证 CUSTOM_HEADER 模式下的设备标识功能
 */

import WebSocket from 'ws';

const ENDPOINT_ID = 'glRJ0WZNf9'; // CUSTOM_HEADER 模式端点（帧头：micu）
const WS_URL = `ws://localhost:3001/ws/${ENDPOINT_ID}`;

console.log('📡 测试 CUSTOM_HEADER 模式下的设备标识功能');
...
ws.on('open', () => {
  ...
  const identifyMsg = {
    type: 'identify',
    deviceId: 'test-device-custom-001',
    deviceName: '测试设备-CUSTOM_HEADER模式',
  };
...
ws.on('message', (data) => {
  ...
  try {
    ...
    if (parsed.type === 'identified') {
      ...
      // 测试成功，关闭连接
      setTimeout(() => {
        console.log('\n✅ 测试通过！CUSTOM_HEADER 模式下设备标识功能正常工作');
        ws.close();
        process.exit(0);
      }, 500);
    }
